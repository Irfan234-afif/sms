...
<template>
  ...
  >
    <span class="text-xs font-normal text-gray-500 dark:text-gray-400">
      Menampilkan
      <span class="font-medium text-gray-900 dark:text-white"> {{ meta.from }}-{{ meta.to }} </span>
      dari
    ...
    <ul class="inline-flex items-stretch -space-x-px">
      ...
      <li v-for="(link, index) in meta.links" :key="index">
        <Link
          v-if="link.url && index != 0 && index != meta.links.length - 1"
          :href="generateURL(link.url)"
          :preserve-scroll="true"
          class="flex items-center justify-center border px-3 py-2 text-xs leading-tight"
          :class="{
