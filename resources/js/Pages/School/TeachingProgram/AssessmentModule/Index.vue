...
const breadcrumbs = [
  ...
  { label: 'Modul Penilaian', href: route('school.teachingProgram.assessmentModule') },
];
</script>
          ...
          <DefaultButton
            ...
            @click="
              openModal({
                ...
                mode: 'assessment-module-create-form',
                maxWidth: 'md',
                data: {},
              })
            "
    ...
    <template #content>
        ...
        <div class="relative overflow-hidden bg-white shadow-md dark:bg-gray-800">
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-left text-xs text-gray-500 dark:text-gray-400">
              ...
              <tbody class="text-xs">
                <tr
                  v-for="(assessment_module, index) in assessment_modules.data"
                  :key="index"
                  ...
                  <td class="whitespace-nowrap px-4 py-3 font-medium text-gray-900 dark:text-white">
                    <div class="flex items-center justify-end space-x-3">
                      <OutlineButton
                        ...
                        @click="
                          openModal({
                            title: 'Sunting Modul Penilaian',
                            mode: 'assessment-module-edit-form',
                            maxWidth: 'md',
                            data: {
                      ...
                      <Link
                        :href="
                          route('school.teachingProgram.assessmentModule.detail', {
                            assessment_module_id: assessment_module.uuid,
                          })
                        "
                      >
                      ...
                      <OutlineButton
                        ...
                        @click="
                          openModal({
                            title: 'Hapus Modul Penilaian',
                            mode: 'assessment-module-delete-confirm',
                            maxWidth: 'md',
                            data: {
                              actionUrl: route('school.teachingProgram.assessmentModule.delete', {
                                assessment_module_id: assessment_module.uuid,
                              }),
                              redirectUrl: route('school.teachingProgram.assessmentModule'),
                              message:
                                'Ingin menghapus Modul Penilaian? Tindakan ini akan memengaruhi data terkait serta penilaian ke depannya. Apakah Anda yakin ingin melanjutkan?',
                            },
                          })
                        "
                      >
          ...
          <Pagination
            ...
            :meta="assessment_modules.meta"
            :links="assessment_modules.links"
          />
        </div>
      </section>
      <!-- modal -->
      <Modal :show="showModal" :property="propertyModal" :maxWidth="propertyModal?.maxWidth" @close="closeModal">
        <template v-slot="{ propertyModal }">
          <SchoolSubjectForm
            v-if="
              propertyModal?.mode == 'assessment-module-edit-form' ||
              propertyModal?.mode == 'assessment-module-create-form'
            "
            :propertyModal="propertyModal"
            @close="closeModal()"
          />
          <DeleteConfirm
            v-if="propertyModal?.mode == 'assessment-module-delete-confirm'"
            :propertyModal="propertyModal"
            @close="closeModal()"
          />
        </template>
