...
export default {
  ...
  data() {
    return {
      ...
      field: {
        ...
        title: {
          label: 'Klub',
          rules: [fieldValidation.isRequired('Klub')],
          error: null,
        },
      },
    };
  ...
  created() {
    ...
    if (mode == 'school-club-edit-form') {
      this.form.school_club_id = this.propertyModal.data.school_club?.uuid;
      this.form.mentor_id = this.propertyModal.data.school_club.mentor;
      if (this.propertyModal.data.school_club.mentor) {
        this.field.mentor_id.options = [this.propertyModal.data.school_club.mentor];
      }
      this.form.title = this.propertyModal.data.school_club?.title;
    }
  },
  methods: {
    optionMentor(search) {
      ...
      axios
        .get(
          route('school.management.schoolClub.optionMentor', {
            search: search,
          }),
        )
        .then((response) => {
    ...
    submit() {
      this.$refs['schoolClubForm'].validate((valid) => {
        if (valid) {
          this.process = true;
          ...
          axios
            .post(route('school.management.schoolClub.save'), requestPayload, {
...
<template>
  <div class="space-y-6 p-5">
    ...
    <div class="px-2">
      <el-form v-if="loaded" ref="schoolClubForm" label-position="top" :model="form" :disabled="process">
        <el-form-item
          class="font-medium"
