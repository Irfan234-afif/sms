...
const breadcrumbs = [
  ...
  { label: 'Klub', href: route('school.management.schoolClub') },
];
</script>
    ...
    <template #content>
      ...
      <section>
        <div class="relative overflow-hidden bg-white shadow-md dark:bg-gray-800">
            ...
            <table class="w-full text-left text-xs text-gray-500 dark:text-gray-400">
              <thead class="bg-gray-50 text-xs text-gray-700 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                  ...
                  <th scope="col" class="p-4">Klub</th>
              ...
              <tbody class="text-xs">
                <tr
                  v-for="(school_club, index) in school_clubs.data"
                  :key="index"
                ...
                >
                  ...
                  <th scope="row" class="whitespace-nowrap px-4 py-3 font-medium text-gray-900 dark:text-white">
                    <div class="flex items-center">
                      {{ school_club.mentor?.profile?.name ?? '-' }}
                    </div>
                  </th>
                    ...
                    <div class="flex items-center justify-end space-x-3">
                      ...
                      <OutlineButton
                        ...
                        @click="
                          openModal({
                            title: 'Hapus Klub',
                            mode: 'school-club-delete-confirm',
                            maxWidth: 'md',
                            data: {
                              actionUrl: route('school.management.schoolClub.delete', {
                                school_club_id: school_club.uuid,
                              }),
                              redirectUrl: route('school.management.schoolClub'),
                              message: 'Ingin menghapus Klub?',
                            },
                          })
                        "
                      >
          ...
          <!-- Pagination -->
          <Pagination :search_params="search_params" :meta="school_clubs.meta" :links="school_clubs.links" />
        </div>
      </section>
      <!-- Modal -->
      <Modal :show="showModal" :property="propertyModal" :maxWidth="propertyModal?.maxWidth" @close="closeModal">
        <template v-slot="{ propertyModal }">
          <SchoolSubjectForm
            v-if="propertyModal?.mode == 'school-club-edit-form' || propertyModal?.mode == 'school-club-create-form'"
            :propertyModal="propertyModal"
            @close="closeModal()"
          />
          <DeleteConfirm
            v-if="propertyModal?.mode == 'school-club-delete-confirm'"
            :propertyModal="propertyModal"
            @close="closeModal()"
          />
        </template>
