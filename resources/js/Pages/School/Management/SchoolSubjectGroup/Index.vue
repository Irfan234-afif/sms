...
const breadcrumbs = [
  ...
  { label: 'Grup Mata Pelajaran', href: route('school.management.schoolSubjectGroup') },
];
</script>
      ...
      >
        ...
        >
          <DefaultButton
            ...
            @click="
              openModal({
                title: 'Grup Mata Pelajaran Baru',
                mode: 'school-subject-group-create-form',
                maxWidth: 'md',
                data: {},
              })
            "
          >
            <div class="flex items-center space-x-1 text-xs">
              ...
              <div>Grup Mata Pelajaran Baru</div>
            </div>
          </DefaultButton>
        </div>
      </div>
    ...
    <template #content>
        ...
        <div class="relative overflow-hidden bg-white shadow-md dark:bg-gray-800">
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-left text-xs text-gray-500 dark:text-gray-400">
              ...
              <tbody class="text-xs">
                <tr
                  v-for="(school_subject_group, index) in school_subject_groups.data"
                  :key="index"
                  ...
                  <td class="whitespace-nowrap px-4 py-3 font-medium text-gray-900 dark:text-white">
                    <div class="flex items-center justify-end space-x-3">
                      <OutlineButton
                        ...
                        @click="
                          openModal({
                            title: 'Sunting Grup Mata Pelajaran',
                            mode: 'school-subject-group-edit-form',
                            maxWidth: 'md',
                            data: {
                              school_subject_group: school_subject_group,
                            },
                          })
                        "
                      >
                      ...
                      <OutlineButton
                        ...
                        @click="
                          openModal({
                            title: 'Hapus Grup Mata Pelajaran',
                            mode: 'school-subject-group-delete-confirm',
                            maxWidth: 'md',
                            data: {
                              actionUrl: route('school.management.schoolSubjectGroup.delete', {
                                school_subject_group_id: school_subject_group.uuid,
                              }),
                              redirectUrl: route('school.management.schoolSubjectGroup'),
                              message: 'Ingin menghapus Grup Mata Pelajaran?',
                            },
                          })
                        "
                      >
          ...
          <Pagination
            ...
            :meta="school_subject_groups.meta"
            :links="school_subject_groups.links"
          />
        </div>
      </section>
      <!-- modal -->
      <Modal :show="showModal" :property="propertyModal" :maxWidth="propertyModal?.maxWidth" @close="closeModal">
        <template v-slot="{ propertyModal }">
          <SchoolSubjectForm
            v-if="
              propertyModal?.mode == 'school-subject-group-edit-form' ||
              propertyModal?.mode == 'school-subject-group-create-form'
            "
            :propertyModal="propertyModal"
            @close="closeModal()"
          />
          <DeleteConfirm
            v-if="propertyModal?.mode == 'school-subject-group-delete-confirm'"
            :propertyModal="propertyModal"
            @close="closeModal()"
          />
        </template>
