...
export default {
  ...
  data() {
    return {
      ...
      loaded: true,
      isValid: false,
      actionRoute: route('office.myProfile.submission.equipment.store'),
      form: {
        submission_id: null,
      ...
      field: {
        ...
        name: {
          label: 'Peralatan / Pemeliharaan',
          rules: [fieldValidation.isRequired('Peralatan / Pemeliharaan')],
          error: null,
        },
        quantity: {
          label: 'Jumlah',
  ...
  created() {
    if (this.propertyModal.mode == 'submission-edit-form') {
      ...
      this.actionRoute = route('office.myProfile.submission.equipment.update');

      this.form.submission_id = submission.uuid;
      this.form.datetime = submission.datetime;
      this.form.name = submission.equipment.items[0].name;
      this.form.due_date = submission.equipment.items[0].due_date;
      this.form.quantity = submission.equipment.items[0].quantity;
      this.form.description = submission.equipment.items[0].description;
    }
  },
  methods: {
    submit(status) {
