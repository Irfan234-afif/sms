...
export default {
  ...
  data() {
    return {
      ...
      loaded: true,
      isValid: false,
      actionRoute: route('office.myProfile.submission.leave.store'),
      form: {
        submission_id: null,
  ...
  created() {
    if (this.propertyModal.mode == 'submission-edit-form') {
      ...
      this.actionRoute = route('office.myProfile.submission.leave.update');

      this.form.submission_id = submission.uuid;
      this.form.datetime = submission.datetime;
      this.form.date_range = [submission.leave.start_date, submission.leave.end_date];
      this.form.description = submission.leave.description;
    }
  },
  methods: {
    submit(status) {
