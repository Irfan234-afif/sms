...
export default {
  ...
  data() {
    return {
      ...
      loaded: true,
      isValid: false,
      actionRoute: route('office.myProfile.submission.resignation.store'),
      form: {
        submission_id: null,
  ...
  created() {
    if (this.propertyModal.mode == 'submission-edit-form') {
      ...
      this.actionRoute = route('office.myProfile.submission.resignation.update');

      this.form.submission_id = submission.uuid;
      this.form.datetime = submission.datetime;
      this.form.date_range = [submission.resignation.entry_date, submission.resignation.out_date];
      this.form.description = submission.resignation.description;
    }
  },
  methods: {
    submit(status) {
      ...
      <el-form v-if="loaded" ref="submissionForm" label-position="top" :model="form" :disabled="process">
        ...
        >
          <el-date-picker
            ...
            type="daterange"
            range-separator="to"
            start-placeholder="Masuk"
            end-placeholder="Keluar"
            format="DD-MM-YYYY"
            value-format="YYYY-MM-DD"
          />
        </el-form-item>
