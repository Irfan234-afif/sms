...
import TestimonialForm from './Form.vue';
import DefaultButton from '@/Components/DefaultButton.vue';
import DeleteConfirm from '@/Components/DeleteConfirm.vue';
const breadcrumbs = [
  { label: 'Yayasan', href: route('office') },
  { label: 'ICC', href: route('office.icc') },
  { label: 'Testimoni', href: route('office.icc.publication.testimonial') },
];
</script>
    ...
    <template #content>
        ...
        <div class="relative overflow-hidden bg-white shadow-md dark:bg-gray-800">
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-left text-xs text-gray-500 dark:text-gray-400">
              ...
              <tbody class="text-xs">
                <tr
                  v-for="(testimonial, index) in testimonials.data"
                  :key="index"
                      ...
                      <OutlineButton
                        ...
                        @click="
                          openModal({
                            ...
                            mode: 'testimonial-delete-confirm',
                            maxWidth: 'md',
                            data: {
                              actionUrl: route('office.icc.publication.testimonial.delete', {
                                testimonial_id: testimonial.uuid,
                              }),
                              redirectUrl: route('office.icc.publication.testimonial'),
                              message: 'Ingin menghapus Testimoni?',
                            },
                          })
                        "
                      >
          ...
          <!-- pagination -->
          <Pagination :search_params="search_params" :meta="testimonials.meta" :links="testimonials.links" />
        </div>
      </section>
      <!-- modal -->
      <Modal :show="showModal" :property="propertyModal" :maxWidth="propertyModal?.maxWidth" @close="closeModal">
        <template v-slot="{ propertyModal }">
          <TestimonialForm
            v-if="propertyModal?.mode == 'testimonial-edit-form' || propertyModal?.mode == 'testimonial-create-form'"
            :propertyModal="propertyModal"
            @close="closeModal()"
          />
          <DeleteConfirm
            v-if="propertyModal?.mode == 'testimonial-delete-confirm'"
            :propertyModal="propertyModal"
            @close="closeModal()"
          />
        </template>
