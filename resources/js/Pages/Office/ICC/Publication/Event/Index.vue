...
const breadcrumbs = [
  ...
  { label: 'Acara', href: route('office.icc.publication.event') },
];
</script>
    ...
    <template #content>
      ...
      <section>
        <div class="relative overflow-hidden bg-white shadow-md dark:bg-gray-800">
                    ...
                    <div class="flex items-center justify-end space-x-3">
                      ...
                      <OutlineButton
                        ...
                        @click="
                          openModal({
                            title: 'Hapus Acara',
                            mode: 'event-delete-confirm',
                            maxWidth: 'md',
                            data: {
                              actionUrl: route('office.icc.publication.event.delete', {
                                event_id: event.uuid,
                              }),
                              redirectUrl: route('office.icc.publication.event'),
                              message: 'Ingin menghapus Acara?',
                            },
                          })
                        "
                      >
          ...
          <!-- Pagination -->
          <Pagination :search_params="search_params" :meta="events.meta" :links="events.links" />
        </div>
      </section>
      <!-- Modal -->
      <Modal :show="showModal" :property="propertyModal" :maxWidth="propertyModal?.maxWidth" @close="closeModal">
        <template v-slot="{ propertyModal }">
          <EventForm
            v-if="propertyModal?.mode == 'event-edit-form' || propertyModal?.mode == 'event-create-form'"
            :propertyModal="propertyModal"
            @close="closeModal()"
          />
          <DeleteConfirm
            v-if="propertyModal?.mode == 'event-delete-confirm'"
            :propertyModal="propertyModal"
            @close="closeModal()"
          />
        </template>
