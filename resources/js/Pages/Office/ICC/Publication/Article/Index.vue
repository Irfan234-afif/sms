...
const breadcrumbs = [
  ...
  { label: 'Artikel', href: route('office.icc.publication.article') },
];
</script>
    ...
    <template #content>
                    ...
                    <div class="flex items-center justify-end space-x-3">
                      ...
                      <OutlineButton
                        ...
                        @click="
                          openModal({
                            title: 'Hapus Artikel',
                            mode: 'post-delete-confirm',
                            maxWidth: 'md',
                            data: {
                              actionUrl: route('office.icc.publication.article.delete', {
                                post_id: post.uuid,
                              }),
                              redirectUrl: route('office.icc.publication.article'),
      ...
      <Modal :show="showModal" :property="propertyModal" :maxWidth="propertyModal?.maxWidth" @close="closeModal">
        <template v-slot="{ propertyModal }">
          ...
          />
          <DeleteConfirm
            v-if="propertyModal?.mode == 'post-delete-confirm'"
            :propertyModal="propertyModal"
            @close="closeModal()"
          />
        </template>
