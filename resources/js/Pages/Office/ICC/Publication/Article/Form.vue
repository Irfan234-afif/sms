...
export default {
  ...
  methods: {
    optionPostCategory(search) {
      ...
      axios
        .get(
          route('office.icc.publication.article.optionPostCategory', {
            search: search,
          }),
        )
        .then((response) => {
    ...
    submit() {
      this.$refs['articleForm'].validate((valid) => {
        if (valid) {
          ...
          axios
            .post(route('office.icc.publication.article.save'), requestPayload, {
            ...
            .catch((error) => {
              ...
              if (error.response?.data?.errors) {
                for (let field in error.response.data.errors) {
                  ...
                  this.$refs['articleForm'].validateField(field);
                }
              }
            })
            .finally(() => {
...
<template>
  <div class="space-y-6 p-5">
    ...
    <div class="px-2">
      <el-form v-if="loaded" ref="articleForm" label-position="top" :model="form" :disabled="process">
        <el-form-item
          class="font-medium"
