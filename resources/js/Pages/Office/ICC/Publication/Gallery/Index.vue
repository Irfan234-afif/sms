...
const breadcrumbs = [
  ...
  { label: 'Galeri', href: route('office.icc.publication.gallery') },
];
</script>
    ...
    <template #content>
      <section>
        <div class="relative overflow-hidden bg-white shadow-md dark:bg-gray-800">
            ...
            <table class="w-full text-left text-xs text-gray-500 dark:text-gray-400">
              <thead class="bg-gray-50 text-xs text-gray-700 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                  ...
                  <th scope="col" class="p-4">Galeri</th>
                    ...
                    <div class="flex items-center justify-end space-x-3">
                      ...
                      <OutlineButton
                        ...
                        @click="
                          openModal({
                            title: 'Hapus Galeri',
                            mode: 'gallery-delete-confirm',
                            maxWidth: 'md',
                            data: {
                              actionUrl: route('office.icc.publication.gallery.delete', {
                                gallery_id: gallery.uuid,
                              }),
                              redirectUrl: route('office.icc.publication.gallery'),
                              message: 'Ingin menghapus Galeri?',
                            },
                          })
                        "
                      >
          ...
          <!-- pagination -->
          <Pagination :search_params="search_params" :meta="galleries.meta" :links="galleries.links" />
        </div>
      </section>
      <!-- modal -->
      <Modal :show="showModal" :property="propertyModal" :maxWidth="propertyModal?.maxWidth" @close="closeModal">
        <template v-slot="{ propertyModal }">
          <GalleryForm
            v-if="propertyModal?.mode == 'gallery-edit-form' || propertyModal?.mode == 'gallery-create-form'"
            :propertyModal="propertyModal"
            @close="closeModal()"
          />
          <DeleteConfirm
            v-if="propertyModal?.mode == 'gallery-delete-confirm'"
            :propertyModal="propertyModal"
            @close="closeModal()"
          />
        </template>
