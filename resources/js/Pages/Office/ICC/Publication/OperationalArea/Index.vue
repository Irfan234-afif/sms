...
const breadcrumbs = [
  ...
  { label: 'Area Operasional', href: route('office.icc.publication.operationalArea') },
];
</script>
      ...
      >
        ...
        >
          <DefaultButton
            ...
            @click="
              openModal({
                title: 'Area Operasional Baru',
                mode: 'operational-area-create-form',
                maxWidth: 'md',
                data: {},
              })
            "
    ...
    <template #content>
        ...
        <div class="relative overflow-hidden bg-white shadow-md dark:bg-gray-800">
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-left text-xs text-gray-500 dark:text-gray-400">
              ...
              <tbody class="text-xs">
                <tr
                  v-for="(operational_area, index) in operational_areas.data"
                  :key="index"
                  ...
                  <td class="whitespace-nowrap px-4 py-3 font-medium text-gray-900 dark:text-white">
                    <div class="flex items-center justify-end space-x-3">
                      <OutlineButton
                        ...
                        @click="
                          openModal({
                            title: 'Sunting Area Operasional',
                            mode: 'operational-area-edit-form',
                            maxWidth: 'md',
                            data: {
                      ...
                      <OutlineButton
                        ...
                        @click="
                          openModal({
                            title: 'Hapus Area Operasional',
                            mode: 'operational-area-delete-confirm',
                            maxWidth: 'md',
                            data: {
                              actionUrl: route('office.icc.publication.operationalArea.delete', {
                                operational_area_id: operational_area.uuid,
                              }),
                              redirectUrl: route('office.icc.publication.operationalArea'),
                              message: 'Ingin menghapus Area Operasional?',
                            },
                          })
                        "
                      >
          ...
          <!-- pagination -->
          <Pagination :search_params="search_params" :meta="operational_areas.meta" :links="operational_areas.links" />
        </div>
      </section>
      <!-- modal -->
      <Modal :show="showModal" :property="propertyModal" :maxWidth="propertyModal?.maxWidth" @close="closeModal">
        <template v-slot="{ propertyModal }">
          <OperationalAreaForm
            v-if="
              propertyModal?.mode == 'operational-area-edit-form' ||
              propertyModal?.mode == 'operational-area-create-form'
            "
            :propertyModal="propertyModal"
            @close="closeModal()"
          />
          <DeleteConfirm
            v-if="propertyModal?.mode == 'operational-area-delete-confirm'"
            :propertyModal="propertyModal"
            @close="closeModal()"
          />
        </template>
