...
import PostCategoryForm from './Form.vue';
import DefaultButton from '@/Components/DefaultButton.vue';
import DeleteConfirm from '@/Components/DeleteConfirm.vue';
const breadcrumbs = [
  { label: 'Yayasan', href: route('office') },
  { label: 'ICC', href: route('office.icc') },
  { label: 'Kategori Posting', href: route('office.icc.publication.postCategory') },
];
</script>
      ...
      >
        ...
        >
          <DefaultButton
            ...
            @click="
              openModal({
                title: 'Kategori Posting Baru',
                mode: 'post-category-create-form',
                maxWidth: 'md',
                data: {},
              })
            "
    ...
    <template #content>
      <section>
        <div class="relative overflow-hidden bg-white shadow-md dark:bg-gray-800">
            ...
            <table class="w-full text-left text-xs text-gray-500 dark:text-gray-400">
              <thead class="bg-gray-50 text-xs text-gray-700 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                  ...
                  <th scope="col" class="p-4">Kategori Posting</th>
                  <th scope="col" class="p-4"></th>
                </tr>
              </thead>
              <tbody class="text-xs">
                <tr
                  v-for="(post_category, index) in post_categories.data"
                  :key="index"
                  ...
                  <td class="whitespace-nowrap px-4 py-3 font-medium text-gray-900 dark:text-white">
                    <div class="flex items-center justify-end space-x-3">
                      <OutlineButton
                        ...
                        @click="
                          openModal({
                            title: 'Sunting Kategori Posting',
                            mode: 'post-category-edit-form',
                            maxWidth: 'md',
                            data: {
                      ...
                      <OutlineButton
                        ...
                        @click="
                          openModal({
                            title: 'Hapus Kategori Posting',
                            mode: 'post-category-delete-confirm',
                            maxWidth: 'md',
                            data: {
                              actionUrl: route('office.icc.publication.postCategory.delete', {
                                post_category_id: post_category.uuid,
                              }),
                              redirectUrl: route('office.icc.publication.postCategory'),
                              message: 'Ingin menghapus Kategori Posting?',
                            },
                          })
                        "
                      >
          ...
          <!-- pagination -->
          <Pagination :search_params="search_params" :meta="post_categories.meta" :links="post_categories.links" />
        </div>
      </section>
      <!-- modal -->
      <Modal :show="showModal" :property="propertyModal" :maxWidth="propertyModal?.maxWidth" @close="closeModal">
        <template v-slot="{ propertyModal }">
          <PostCategoryForm
            v-if="
              propertyModal?.mode == 'post-category-edit-form' || propertyModal?.mode == 'post-category-create-form'
            "
            :propertyModal="propertyModal"
            @close="closeModal()"
          />
          <DeleteConfirm
            v-if="propertyModal?.mode == 'post-category-delete-confirm'"
            :propertyModal="propertyModal"
            @close="closeModal()"
          />
        </template>
