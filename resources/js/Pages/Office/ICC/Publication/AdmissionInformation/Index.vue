...
import admissionInformationForm from './Form.vue';
import DefaultButton from '@/Components/DefaultButton.vue';
import DeleteConfirm from '@/Components/DeleteConfirm.vue';
const breadcrumbs = [
  { label: 'Yayasan', href: route('office') },
  { label: 'ICC', href: route('office.icc') },
  { label: 'Informasi Pendaftaran', href: route('office.icc.publication.admissionInformation') },
];
</script>
      ...
      >
        ...
        >
          <DefaultButton
            ...
            @click="
              openModal({
                title: 'Informasi Pendaftaran Baru',
                mode: 'admission-information-create-form',
                maxWidth: '5xl',
                data: {},
              })
            "
          >
            <div class="flex items-center space-x-1 text-xs">
              ...
              <div>Informasi Pendaftaran Baru</div>
            </div>
          </DefaultButton>
        </div>
      </div>
    ...
    <template #content>
      <section>
        <div class="relative overflow-hidden bg-white shadow-md dark:bg-gray-800">
                  ...
                  <td class="whitespace-nowrap px-4 py-3 font-medium text-gray-900 dark:text-white">
                    <div class="flex items-center justify-end space-x-3">
                      <OutlineButton
                        ...
                        @click="
                          openModal({
                            title: 'Sunting Informasi Pendaftaran',
                            mode: 'admission-information-edit-form',
                            maxWidth: '5xl',
                            data: {
                              post: post,
                            },
                      ...
                      <OutlineButton
                        ...
                        @click="
                          openModal({
                            title: 'Hapus Informasi Pendaftaran',
                            mode: 'admission-information-delete-confirm',
                            maxWidth: 'md',
                            data: {
                              actionUrl: route('office.icc.publication.admissionInformation.delete', {
                                post_id: post.uuid,
                              }),
                              redirectUrl: route('office.icc.publication.admissionInformation'),
                              message: 'Ingin menghapus Informasi Pendaftaran?',
                            },
                          })
                        "
                      >
          ...
          <!-- pagination -->
          <Pagination :search_params="search_params" :meta="posts.meta" :links="posts.links" />
        </div>
      </section>
      <!-- modal -->
      <Modal :show="showModal" :property="propertyModal" :maxWidth="propertyModal?.maxWidth" @close="closeModal">
        <template v-slot="{ propertyModal }">
          <admissionInformationForm
            v-if="
              propertyModal?.mode == 'admission-information-edit-form' ||
              propertyModal?.mode == 'admission-information-create-form'
            "
            :propertyModal="propertyModal"
            @close="closeModal()"
          />
          <DeleteConfirm
            v-if="propertyModal?.mode == 'admission-information-delete-confirm'"
            :propertyModal="propertyModal"
            @close="closeModal()"
          />
        </template>
