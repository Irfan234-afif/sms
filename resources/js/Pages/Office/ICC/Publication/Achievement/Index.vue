...
import AchievementForm from './Form.vue';
import DefaultButton from '@/Components/DefaultButton.vue';
import DeleteConfirm from '@/Components/DeleteConfirm.vue';
const breadcrumbs = [
  { label: 'Yayasan', href: route('office') },
  { label: 'ICC', href: route('office.icc') },
  { label: 'Pencapaian', href: route('office.icc.publication.achievement') },
];
</script>
    ...
    <template #content>
      <section>
        <div class="relative overflow-hidden bg-white shadow-md dark:bg-gray-800">
                      ...
                      <OutlineButton
                        ...
                        @click="
                          openModal({
                            ...
                            mode: 'achievement-delete-confirm',
                            maxWidth: 'md',
                            data: {
                              actionUrl: route('office.icc.publication.achievement.delete', {
                                achievement_id: achievement.uuid,
                              }),
                              redirectUrl: route('office.icc.publication.achievement'),
                              message: 'Ingin menghapus Pencapaian?',
                            },
                          })
                        "
                      >
          ...
          <!-- pagination -->
          <Pagination :search_params="search_params" :meta="achievements.meta" :links="achievements.links" />
        </div>
      </section>
      <!-- modal -->
      <Modal :show="showModal" :property="propertyModal" :maxWidth="propertyModal?.maxWidth" @close="closeModal">
        <template v-slot="{ propertyModal }">
          <AchievementForm
            v-if="propertyModal?.mode == 'achievement-edit-form' || propertyModal?.mode == 'achievement-create-form'"
            :propertyModal="propertyModal"
            @close="closeModal()"
          />
          <DeleteConfirm
            v-if="propertyModal?.mode == 'achievement-delete-confirm'"
            :propertyModal="propertyModal"
            @close="closeModal()"
          />
        </template>
