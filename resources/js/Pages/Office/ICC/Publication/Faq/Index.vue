...
const breadcrumbs = [
  ...
  { label: 'FAQ', href: route('office.icc.publication.faq') },
];
</script>
    ...
    <template #content>
      ...
      <section>
        <div class="relative overflow-hidden bg-white shadow-md dark:bg-gray-800">
                    ...
                    <div class="flex items-center justify-end space-x-3">
                      ...
                      <OutlineButton
                        ...
                        @click="
                          openModal({
                            title: 'Hapus FAQ',
                            mode: 'faq-delete-confirm',
                            maxWidth: 'md',
                            data: {
                              actionUrl: route('office.icc.publication.faq.delete', {
                                faq_id: faq.uuid,
                              }),
                              redirectUrl: route('office.icc.publication.faq'),
                              message: 'Ingin menghapus FAQ?',
                            },
                          })
                        "
                      >
          ...
          <!-- Pagination -->
          <Pagination :search_params="search_params" :meta="faqs.meta" :links="faqs.links" />
        </div>
      </section>
      <!-- Modal -->
      <Modal :show="showModal" :property="propertyModal" :maxWidth="propertyModal?.maxWidth" @close="closeModal">
        <template v-slot="{ propertyModal }">
          <FaqForm
            v-if="propertyModal?.mode == 'faq-edit-form' || propertyModal?.mode == 'faq-create-form'"
            :propertyModal="propertyModal"
            @close="closeModal()"
          />
          <DeleteConfirm
            v-if="propertyModal?.mode == 'faq-delete-confirm'"
            :propertyModal="propertyModal"
            @close="closeModal()"
          />
        </template>
