...
const breadcrumbs = [
  ...
  { label: 'Karir', href: route('office.icc.publication.career') },
];
</script>
    ...
    <template #content>
      ...
      <section>
        <div class="relative overflow-hidden bg-white shadow-md dark:bg-gray-800">
            ...
            <table class="w-full text-left text-xs text-gray-500 dark:text-gray-400">
              <thead class="bg-gray-50 text-xs text-gray-700 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                  ...
                  <th scope="col" class="p-4">Karir</th>
                    ...
                    <div class="flex items-center justify-end space-x-3">
                      ...
                      <OutlineButton
                        ...
                        @click="
                          openModal({
                            title: 'Hapus Karir',
                            mode: 'career-delete-confirm',
                            maxWidth: 'md',
                            data: {
                              actionUrl: route('office.icc.publication.career.delete', {
                                career_id: career.uuid,
                              }),
                              redirectUrl: route('office.icc.publication.career'),
                              message: 'Ingin menghapus Karir?',
                            },
                          })
                        "
                      >
          ...
          <!-- Pagination -->
          <Pagination :search_params="search_params" :meta="careers.meta" :links="careers.links" />
        </div>
      </section>
      <!-- Modal -->
      <Modal :show="showModal" :property="propertyModal" :maxWidth="propertyModal?.maxWidth" @close="closeModal">
        <template v-slot="{ propertyModal }">
          <SchoolSubjectForm
            v-if="propertyModal?.mode == 'career-edit-form' || propertyModal?.mode == 'career-create-form'"
            :propertyModal="propertyModal"
            @close="closeModal()"
          />
          <DeleteConfirm
            v-if="propertyModal?.mode == 'career-delete-confirm'"
            :propertyModal="propertyModal"
            @close="closeModal()"
          />
        </template>
