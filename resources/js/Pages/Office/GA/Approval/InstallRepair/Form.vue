...
export default {
  ...
  data() {
    return {
      ...
      loaded: true,
      isValid: false,
      actionRoute: route('office.ga.approval.installRepair.store'),
      form: {
        submission_id: null,
  ...
  created() {
    if (this.propertyModal.mode == 'submission-edit-form') {
      ...
      this.actionRoute = route('office.ga.approval.installRepair.update');
  ...
  methods: {
    optionAssigned(search) {
      ...
      axios
        .get(
          route('office.ga.approval.installRepair.optionAssigned', {
            search: search,
          }),
        )
        .then((response) => {
