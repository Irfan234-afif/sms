          ...
          <div class="mx-auto max-w-5xl">
                  ...
                  <header class="not-format mb-4 lg:mb-6">
                    <address class="mb-6 flex items-center not-italic">
                      <div class="mr-3 inline-flex items-center text-sm text-gray-900 dark:text-white">
                        <img class="mr-4 h-12 w-12 rounded-full" :src="article.data.author.avatar_path" alt="" />
                        <div>
                          ...
                          <p class="text-base text-gray-500 dark:text-gray-400">
                            <time>
                              {{
                                new Date(article.data.published_at).toLocaleDateString('en-US', {
                                  month: 'short',
                                  day: '2-digit',
                                  year: 'numeric',
                                })
                    ...
                    <div>
                      <img class="w-full rounded-lg" :src="article.data.thumbnail_path" alt="" />
                    </div>
                  </header>
                  <p>
                    <PreviewContent :content="article.data.content" />
                  </p>
                </article>
              </div>
            </main>
            <aside class="bg-gray-50 py-8 dark:bg-gray-800">
              <div class="mx-auto max-w-screen-xl px-4">
                <h2 class="mb-8 text-xl font-bold text-gray-900 dark:text-white">Artikel Lainnya</h2>
                <div class="grid gap-12 sm:grid-cols-2 lg:grid-cols-4">
                  <article v-for="(more_article, index) in more_articles.data" :key="index" class="max-w-xs">
                    <Link
                      :href="
                        route('article.detail', {
                          slug: more_article.slug,
                        })
                      "
                    >
                      <img :src="more_article.thumbnail_path" class="mb-5 h-32 w-full rounded-lg object-cover" />
                    </Link>
