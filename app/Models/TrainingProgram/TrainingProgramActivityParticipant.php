...
class TrainingProgramActivityParticipant extends Model
{
    ...
    protected $table = 'tp_activity_participants';
    ...
    public function calculateOverallProgress(): void
    {
        ...
        // Calculate average progress percentage across all phases
        $phaseProgressList = $this->phaseProgress()->get();

        $totalProgress = 0;
        foreach ($phaseProgressList as $phaseProgress) {
            $totalProgress += $phaseProgress->progress_percentage;
        }

        $progress = $phaseProgressList->count() > 0 ? ($totalProgress / $phaseProgressList->count()) : 0;        
